...
**                                                                    **
** Modified to make the element valid for 2D 3DOF analyses by:        **
**   Andronikos Skiadopoulos (andronikos.skiadopoulos@epfl.ch)        **
**   Dimitrios G. Lignos (dimitrios.lignos@epfl.ch)                   **
**                                                                    **
** ****************************************************************** */
                                                                        
// $Revision: 2.00 $
// $Date: 2009-10-13 21:14:21 $
// $Source: /usr/local/cvs/OpenSees/SRC/element/FourNodeQuad/FourNodeQuad2D3DOF.cpp,v $

// Written: MHS
// Created: Feb 2000
// Revised: Dec 2000 for efficiency
// Revised: Aug 2020 for 2D, 3DOF analyses (by Skiadopoulos and Lignos)
//
// Description: This file contains the class definition for FourNodeQuad2D3DOF.

#include <FourNodeQuad2D3DOF.h>
#include <Node.h>
...
void* OPS_FourNodeQuad2D3DOF()
{
	...
	opserr << "Want: element FourNodeQuad2D3DOF eleTag? iNode? jNode? kNode? lNode? thk? type? matTag? <pressure? rho? b1? b2?>\n";
	return 0;
    }

    // FourNodeQuad2D3DOFId, iNode, jNode, kNode, lNode
    int idata[5];
    int num = 5;
	...
	opserr << "\nFourNodeQuad2D3DOF element: " << idata[0] << endln;
	return 0;
    }
    ...
    return new FourNodeQuad2D3DOF(idata[0],idata[1],idata[2],idata[3],idata[4],
			                *mat,type,thk,data[0],data[1],data[2],data[3]);
}


double FourNodeQuad2D3DOF::matrixData[64];
double FourNodeQuad2D3DOF::matrixData_1212[144];
Matrix FourNodeQuad2D3DOF::K(matrixData, 8, 8);
Matrix FourNodeQuad2D3DOF::K_1212(matrixData_1212, 12, 12);
Vector FourNodeQuad2D3DOF::P(8);
Vector FourNodeQuad2D3DOF::P_12(12);
double FourNodeQuad2D3DOF::shp[3][4];
double FourNodeQuad2D3DOF::pts[4][2];
double FourNodeQuad2D3DOF::wts[4];

FourNodeQuad2D3DOF::FourNodeQuad2D3DOF(int tag, int nd1, int nd2, int nd3, int nd4,
			   NDMaterial &m, const char *type, double t,
			   double p, double r, double b1, double b2)
:Element (tag, ELE_TAG_FourNodeQuad2D3DOF), 
  theMaterial(0), connectedExternalNodes(4), 
 Q(12), pressureLoad(12), thickness(t), applyLoad(0), pressure(p), rho(r), Ki(0)
{
	//std::cout << "starting to wait forever" << std::endl;
	//int stupidVal = 0;
	//while (stupidVal != 1) {
	//	stupidVal = 0;
	//}
	...
	if (strcmp(type,"PlaneStrain") != 0 && strcmp(type,"PlaneStress") != 0
	  ...
	  opserr << "FourNodeQuad2D3DOF::FourNodeQuad2D3DOF -- improper material type: " << type << "for FourNodeQuad2D3DOF\n";
	  exit(-1);
	}
    ...
    if (theMaterial == 0) {
      opserr << "FourNodeQuad2D3DOF::FourNodeQuad2D3DOF - failed allocate material model pointer\n";
      exit(-1);
    }
    ...
    for (i = 0; i < 4; i++) {
      ...
      // Check allocation
      if (theMaterial[i] == 0) {
	opserr << "FourNodeQuad2D3DOF::FourNodeQuad2D3DOF -- failed to get a copy of material model\n";
	exit(-1);
      }
    }
...
FourNodeQuad2D3DOF::FourNodeQuad2D3DOF()
:Element (0,ELE_TAG_FourNodeQuad2D3DOF),
  theMaterial(0), connectedExternalNodes(4), 
 Q(12), pressureLoad(12), thickness(0.0), applyLoad(0), pressure(0.0), Ki(0)
{
...
int
FourNodeQuad2D3DOF::getNumExternalNodes() const
{
    return 4;
}

const ID&
FourNodeQuad2D3DOF::getExternalNodes()
{
...
Node **
FourNodeQuad2D3DOF::getNodePtrs(void) 
{
  return theNodes;
}

int
FourNodeQuad2D3DOF::getNumDOF()
{
    return 12;
}

void
FourNodeQuad2D3DOF::setDomain(Domain *theDomain)
{
	...
	//opserr << "FATAL ERROR FourNodeQuad2D3DOF (tag: %d), node not found in domain",
	//	this->getTag());
	...
	//opserr << "FATAL ERROR FourNodeQuad2D3DOF (tag: %d), has differing number of DOFs at its nodes",
	//	this->getTag());
...
int
FourNodeQuad2D3DOF::commitState()
{
    int retVal = 0;

    // call element commitState to do any base class stuff
    if ((retVal = this->Element::commitState()) != 0) {
      opserr << "FourNodeQuad2D3DOF::commitState () - failed in base class";
    }    
...
int
FourNodeQuad2D3DOF::revertToLastCommit()
{
    int retVal = 0;
...
int
FourNodeQuad2D3DOF::revertToStart()
{
    int retVal = 0;
...
const Matrix&
FourNodeQuad2D3DOF::getTangentStiff()
{

	K.Zero();
	K_1212.Zero(); // Put zeros in all elements of the matrix. 
	for (int i = 0; i < 12; i++) {
		K_1212(i,i) = 10^(-20); // Avoid zeros in the diagonal so that inversion issues are prohibited.
	}
	...
	for (int i = 0; i < 8; i ++) {
	   for (int j = 0; j < 8; j ++) {
	     K_1212(i + i / 2,j + j / 2) = K(i,j);
		 //opserr << "i = " << i << " " << "j = " << j << endln; // should be deleted afterwords
	  }
	}	
	//opserr << K << endln; // should be deleted afterwords
	//opserr << K_1212 << endln; // should be deleted afterwords
	return K_1212;
}


const Matrix&
FourNodeQuad2D3DOF::getInitialStiff()
{
  if (Ki != 0)
    return *Ki;
...
void
FourNodeQuad2D3DOF::zeroLoad(void)
{
	Q.Zero();
...
int 
FourNodeQuad2D3DOF::addLoad(ElementalLoad *theLoad, double loadFactor)
{
	...
	if (type == LOAD_TAG_SelfWeight) {
		...
		opserr << "FourNodeQuad2D3DOF::addLoad - load type unknown for ele with tag: " << this->getTag() << endln;
		return -1;
	} 
...
int 
FourNodeQuad2D3DOF::addInertiaLoadToUnbalance(const Vector &accel)
{
  int i;
  static double rhoi[4];
  double sum = 0.0;
  ...
  if (3 != Raccel1.Size() || 3 != Raccel2.Size() || 3 != Raccel3.Size() || // I only changed this line in this function, since it does not return anything
      3 != Raccel4.Size()) {
    opserr << "FourNodeQuad2D3DOF::addInertiaLoadToUnbalance matrix and vector sizes are incompatible\n";
    return -1;
  }
...
const Vector&
FourNodeQuad2D3DOF::getResistingForce()
{
	P.Zero();
	P_12.Zero();
     for (int i = 0; i < 12; i++) {
...
const Vector&
FourNodeQuad2D3DOF::getResistingForceIncInertia()
{
	int i;
	static double rhoi[4];
	double sum = 0.0;
	...
	if (sum == 0.0) {
	  ...
	  if (betaK != 0.0 || betaK0 != 0.0 || betaKc != 0.0)
	    P_12 += this->getRayleighDampingForces();
	  return P_12;
	}
	...
	if (alphaM != 0.0 || betaK != 0.0 || betaK0 != 0.0 || betaKc != 0.0)
	  P_12 += this->getRayleighDampingForces();
	return P_12;
	// return FourNodeQuad2D3DOF::vector_8_to_12(P);
}

int
FourNodeQuad2D3DOF::sendSelf(int commitTag, Channel &theChannel)
{
  int res = 0;
  ...
  if (res < 0) {
    opserr << "WARNING FourNodeQuad2D3DOF::sendSelf() - " << this->getTag() << " failed to send Vector\n";
    return res;
  }	      
  ...
  if (res < 0) {
    opserr << "WARNING FourNodeQuad2D3DOF::sendSelf() - " << this->getTag() << " failed to send ID\n";
    return res;
  }
  ...
  for (i = 0; i < 4; i++) {
    ...
    if (res < 0) {
      opserr << "WARNING FourNodeQuad2D3DOF::sendSelf() - " << this->getTag() << " failed to send its Material\n";
      return res;
    }
  }
...
int
FourNodeQuad2D3DOF::recvSelf(int commitTag, Channel &theChannel,
                       FEM_ObjectBroker &theBroker)
{
  ...
  if (res < 0) {
    opserr << "WARNING FourNodeQuad2D3DOF::recvSelf() - failed to receive Vector\n";
    return res;
  }
  ...
  if (res < 0) {
    opserr << "WARNING FourNodeQuad2D3DOF::recvSelf() - " << this->getTag() << " failed to receive ID\n";
    return res;
  }
  ...
  if (theMaterial == 0) {
    ...
    theMaterial = new NDMaterial *[4];
    if (theMaterial == 0) {
      opserr << "FourNodeQuad2D3DOF::recvSelf() - Could not allocate NDMaterial* array\n";
      return -1;
    }
    for (int i = 0; i < 4; i++) {
      ...
      if (theMaterial[i] == 0) {
	    opserr << "FourNodeQuad2D3DOF::recvSelf() - Broker could not create NDMaterial of class type " << matClassTag << endln;
	    return -1;
      }
      ...
      if (res < 0) {
        opserr << "FourNodeQuad2D3DOF::recvSelf() - material " << i << "failed to recv itself\n";
	    return res;
      }
    }
  }
  ...
  // materials exist , ensure materials of correct type and recvSelf on them
    ...
    for (int i = 0; i < 4; i++) {
      ...
      if (theMaterial[i]->getClassTag() != matClassTag) {
	    ...
	    theMaterial[i] = theBroker.getNewNDMaterial(matClassTag);
	    if (theMaterial[i] == 0) {
          opserr << "FourNodeQuad2D3DOF::recvSelf() - material " << i << "failed to create\n";
	      return -1;
	    }
      }
...
void
FourNodeQuad2D3DOF::Print(OPS_Stream &s, int flag)
{
  if (flag == 2) {

    s << "#FourNodeQuad2D3DOF\n";
...
/*   if (flag == OPS_PRINT_CURRENTSTATE) {
    s << "\nFourNodeQuad2D3DOF, element id:  " << this->getTag() << endln;
	s << "\tConnected external nodes:  " << connectedExternalNodes;
	s << "\tthickness:  " << thickness << endln;
	s << "\tsurface pressure:  " << pressure << endln;
...
int
FourNodeQuad2D3DOF::displaySelf(Renderer &theViewer, int displayMode, float fact, const char **modes, int numMode)
{
...
Response*
FourNodeQuad2D3DOF::setResponse(const char **argv, int argc, 
			  OPS_Stream &output)
{
  Response *theResponse =0;

  output.tag("ElementOutput");
  output.attr("eleType","FourNodeQuad2D3DOF");
...
int 
FourNodeQuad2D3DOF::getResponse(int responseID, Information &eleInfo)
{
  if (responseID == 1) {
...
int
FourNodeQuad2D3DOF::setParameter(const char **argv, int argc, Parameter &param)
{
  if (argc < 1)
    return -1;
...
int
FourNodeQuad2D3DOF::updateParameter(int parameterID, Information &info)
{
	int res = -1;
		int matRes = res;
  switch (parameterID) {
...
double FourNodeQuad2D3DOF::shapeFunction(double xi, double eta)
{
...
void 
FourNodeQuad2D3DOF::setPressureLoadAtNodes(void)
{
        pressureLoad.Zero();
